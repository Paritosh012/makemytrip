            ...
            <table>
              ...
              <tbody>
                {bookings.map((b) => (
                  <tr key={b._id}>
                    <td>
                      <div style={{ fontWeight: 500 }}>{b.packageId?.title || '—'}</div>
                      <div className="text-muted">📍 {b.packageId?.destination || '—'}</div>
                    </td>
                    <td>₹{b.price?.toLocaleString('en-IN')}</td>
                    <td>{statusBadge(b.status)}</td>
                    <td>
                      <span className={`badge ${b.paymentStatus === 'SUCCESS' ? 'badge-green' : b.paymentStatus === 'FAILED' ? 'badge-red' : 'badge-yellow'}`}>
                        {b.paymentStatus}
                      </span>
                    </td>
